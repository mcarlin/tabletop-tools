...
def _unpack_objects(
    ...
    for obj in objects:
        ...
        if obj_script.strip():
            script_path.write_text(bundler.unbundle(to_unix(obj_script), config))
        ...
        if contained_objects is not None:
            _unpack_objects(
                contained_objects, path.joinpath("contained"), config, bundler
            )
...
def unpack(*, savegame: Dict[str, Any], config: Config) -> None:
    ...
    config.global_script.write_text(bundler.unbundle(global_script, config))
    ...
    _unpack_objects(savegame.pop("ObjectStates"), config.objects, config, bundler)
